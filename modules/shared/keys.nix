...
in {
  options.local.keys = {
    ...
    enable_authorized_access = lib.mkOption {
      description =
        "Allow key-authenticated SSH access to user config.local.keys.authorized_user";
      default = false;
    };
    ...
    authorized_user_key = lib.mkOption {
      ...
      default = builtins.readFile
        "${flake-root}/pubkeys/ssh/id_ed25519.pub.mbv-mba.mvilladsen";
    };
  };
  ...
  config.programs.ssh.knownHosts = lib.mkIf cfg.enable ({
    ...
    name = host;
    value = { publicKeyFile = getPubHostKey host; };
  ...
  config.users.users = lib.mkIf cfg.enable_authorized_access {
    ${cfg.authorized_user}.openssh.authorizedKeys.keys =
      [ cfg.authorized_user_key ];
  };
}
