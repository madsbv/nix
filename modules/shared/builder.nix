...
let cfg = config.local.builder;
in {
  options.local.builder = {
    enableLocalBuilder = lib.mkOption {
      description =
        "If true, create a local user with minimal permissions to act as access point for using this machine as a remote builder.";
      default = true;
    };
    enableRemoteBuilders = lib.mkOption {
      description =
        "Enable the use of other nodes as remote builders over Tailscale";
      default = true;
    };
  };
  ...
  config = {
    ...
    users = lib.mkIf cfg.enableLocalBuilder {
      ...
      groups.builder = { members = [ "builder" ]; };
    };
