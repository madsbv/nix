...
in
{
  options.local.shell.enable = lib.mkEnableOption "shell";
  config = {
    ...
    # TODO: Extract into separate file that can be imported as pure homeManager module, while still sharing shell aliases and such
    home-manager.sharedModules = lib.mkIf config.local.homeManager.enable [
      (
        {
          inputs,
          pkgs,
        ...
        in
        {
          options.local.shell.enable = lib.mkEnableOption "home-manager shell config";
