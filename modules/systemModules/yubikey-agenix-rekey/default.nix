...
in
{
  options.local.agenix = {
    ...
    hostPubkey = lib.mkOption {
      default = flake-root + "/pubkeys/ssh/ssh_host_ed25519_key.pub.${hostname}";
    };
    ...
    extraEncryptionPubkeys = lib.mkOption {
      default = [ (flake-root + "/pubkeys/yubikey/recipients.pub") ];
    };
  };
  config.age = lib.mkIf cfg.enable {
    inherit (cfg) identityPaths;
    rekey = {
      inherit (cfg) hostPubkey masterIdentities extraEncryptionPubkeys;
      storageMode = "local";
